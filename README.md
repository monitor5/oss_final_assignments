...
│   ├── storage.py         # 이름+패스워드 결과 저장/불러오기 (storage.json)
│   ├── data/              # storage.json (호스트 바인드 마운트, gitignore)
│   ├── requirements.txt
│   ├── Dockerfile
│   └── .dockerignore
...
| POST | `/save` | 이름+패스워드로 추천 결과 저장 |
| POST | `/load` | 이름+패스워드로 저장된 결과 불러오기 |
| GET | `/docs` | Swagger 자동 문서 |

> 저장/불러오기: 이름과 패스워드(SHA-256 해시 저장)로 추천 결과를 보관합니다.
> 저장할 때마다 `storage.json` 에 기록되며, 이 파일은 docker-compose 의
> **호스트 바인드 마운트(`./back/data`)** 로 보관됩니다.
> 따라서 컨테이너 삭제·이미지 재빌드·`docker compose down`(`-v` 포함)·서버 재부팅에도
> 데이터가 그대로 유지됩니다. 추가로 `/save` · `/load` 결과를 JSON 파일로
> 백업/복원(`/restore`)할 수도 있습니다.
